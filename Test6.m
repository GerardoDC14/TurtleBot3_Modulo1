...
leader_speed = 0.02; % Adjusted leader speed for smoother movement
...
max_linear_velocity = 0.04;   % [m/s] TurtleBot3 max linear velocity
max_angular_velocity = 0.6;  % [rad/s] TurtleBot3 max angular velocity

%% Desired Offset for Formation (Parallel Following)

% Define lateral offsets (meters)
offset_distance_robot1 = 0.08;  % Distance from the leader's path for Robot 1
offset_distance_robot2 = 0.08;  % Distance from the leader's path for Robot 2

% Define the side for each robot: Robot1 on the left, Robot2 on the right
side_robot1 = -1;  % 1 for left, -1 for right
side_robot2 = 1; % -1 for right, 1 for left
...
while elapsed_time < max_time
    ...
    %% Desired Positions for Robots (Parallel Formation)

    % Calculate the desired position offsets based on leader's orientation
    % Perpendicular direction to the leader's heading
    perp_theta = leader.theta + pi/2;

    % Desired positions
    desired_x1 = leader.x + offset_distance_robot1 * cos(perp_theta) * side_robot1;
    desired_y1 = leader.y + offset_distance_robot1 * sin(perp_theta) * side_robot1;

    desired_x2 = leader.x + offset_distance_robot2 * cos(perp_theta) * side_robot2;
    desired_y2 = leader.y + offset_distance_robot2 * sin(perp_theta) * side_robot2;

    % Desired orientations can be aligned with the leader's orientation
